...
* [X]  在搜索页，为三个平台加入搜索历史功能，以及清除历史记录按钮。
* [X]  在三个平台的我的库的音乐歌单的单曲加入“从歌单移出“功能
* [X]  在三个平台我的库界面加载完成后的歌单里的歌曲列表的布局中，全部播放按钮和歌曲列表中间有一大块空隙。修复这个问题。
* [X]  三个平台我的库界面在初次打开时直接自动打开第一个歌单
* [X]  在设置增加一个功能：2分钟，5分钟，10分钟没有操作app的时候自动跳转到待机页standby page。默认时间为5分钟。
* [X]  将底部控制栏ui的颜色从绿色改为白色。

Mac OS

* [X]  macos 顶部状态栏歌曲显示
* [X]  接入macos系统媒体播放管线，也即macos的系统媒体控制能抓取到app音乐播放情况和进行控制。
* [X]  编辑macos系统状态栏左上角菜单内容。
* [X]  设计app icon。
* [X]  我这个app希望有自动接收更新功能，做一个。
* [X]  我想把这个app的titlebar变成透明的，同时让app内容的上边缘扩张到titlebar顶部，同时保留系统默认红绿灯。这样可以在视觉上将titlebar隐藏。是否可行？
